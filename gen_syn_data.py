...
def instagram_transform(img, xform_name):
    ...
    if img.shape[0] <= 3:
        CHW_flag=True
        img = np.transpose(img,[1,2,0]) #trans to [H,W,C]
    ...
    rst_img = X_obj.get_data()
    if CHW_flag:
        rst_img = np.transpose(rst_img,[2,0,1]) #trans back to [C,H,W]
